<template>
  <div class="editor-toolbar-item" @click="openDialog">
    <v-tooltip activator="parent" text="插入视频" location="bottom" />
    <v-icon :icon="mdiVideo" />
  </div>

  <v-dialog v-model="dialog" max-width="440">
    <v-card class="rounded-3xl">
      <v-card-title class="pt-6 pb-0 text-lg font-bold">插入视频</v-card-title>

      <v-card-text class="pb-2">
        <!-- upload/url tabs — only when uploadVideo is provided -->
        <v-tabs v-if="props.uploadVideo" v-model="tab" class="-mx-1 mb-4" density="compact">
        ...
        <template v-else>
          ...
          >
            ...
            <p class="text-sm text-gray-500">点击选择或拖拽视频到此处</p>
            <p
              v-if="uploadFile"
...
import { mdiUpload, mdiVideo } from "@mdi/js"
import { computed, ref } from "vue"

import type { EditorToolbarProps } from "@/components/Editor/Toolbar/types"

interface VideoToolbarProps extends EditorToolbarProps {
  uploadVideo?: (file: File) => Promise<string>
}

const props = defineProps<VideoToolbarProps>()
...
function openDialog() {
  tab.value = props.uploadVideo ? "upload" : "url"
  urlInput.value = ""
...
async function confirm() {
  ...
  if (!uploadFile.value || !props.uploadVideo) return
  uploading.value = true
  try {
    const url = await props.uploadVideo(uploadFile.value)
    insert(url)
  } catch {
...
function insert(src: string) {
  props.editor?.chain().focus().insertContent({ type: "video", attrs: { src } }).run()
  dialog.value = false
}
</script>
